		...
		case TBCD_CHANNEL: {
			...
			if (pDC) {
				...
				if (GetStyle() & TBS_VERT) {
					...
					if (!IsActive) {
						pDC->FillSolidRect(CRect(rect.left, rect.top, rect.right, rect.bottom), Theme::Window);
					}
					else {
						pDC->FillSolidRect(CRect(rect.left, rect.top, rect.right, rect.bottom), Theme::Control);
					}
