...
BOOL Dialer::OnInitDialog()
{
	...
	m_ButtonCall.m_FaceColor = Theme::Selection;
...
void Dialer::RebuildButtons(bool init)
{
	...
	{
		// Voicemail number setting removed; the button stays a hidden layout
		// anchor for the REC/CONF/AA/AC/FWD/DND row below.
		m_isButtonVoicemailVisible = false;
...
void Dialer::Action(DialerActions action)
{
	...
	number.Trim();
	if (!number.IsEmpty()) {
		bool res = mainDlg->MakeCall(number, action == ACTION_VIDEO_CALL);
		if (res) {
