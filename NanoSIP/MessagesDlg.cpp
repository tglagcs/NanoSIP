...
BOOL MessagesDlg::OnInitDialog()
{
	...
	((CRichEditCtrl*)GetDlgItem(IDC_MESSAGES_LIST))->SetBackgroundColor(FALSE, Theme::Window);
	((CRichEditCtrl*)GetDlgItem(IDC_MESSAGE))->SetBackgroundColor(FALSE, Theme::Control);
	{
		CHARFORMAT cfInput;
		ZeroMemory(&cfInput, sizeof(cfInput));
		cfInput.cbSize = sizeof(cfInput);
		cfInput.dwMask = CFM_COLOR;
		cfInput.crTextColor = Theme::Text;
		((CRichEditCtrl*)GetDlgItem(IDC_MESSAGE))->SetDefaultCharFormat(cfInput);
	}
...
void MessagesDlg::AddMessage(MessagesContact* messagesContact, CString message, int type, BOOL blockForeground, CTime* pTime)
{
	...
	cf.crTextColor = type == MSIP_MESSAGE_TYPE_SYSTEM ? Theme::TextDim : color;
	cf.dwEffects = 0;
