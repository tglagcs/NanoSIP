...
void CButtonDialer::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	// The dialer keys sit edge-to-edge in the dialog template, so a flat
	// FillSolidRect over the whole item rect makes them look like one
	// solid block. Leave a thin margin showing the dialog's background
	// (gap between keys) and round the corners of the fill, matching the
	// separated/rounded look of the original (light theme) keypad.
	dc.FillSolidRect(rt, Theme::Window);
	CRect rtFill(rt);
	int margin = max(1, MulDiv(1, dpiY, 96));
	rtFill.DeflateRect(margin, margin);
	int radius = MulDiv(6, dpiY, 96);
	CBrush brush(fill);
	CPen pen(PS_NULL, 0, RGB(0, 0, 0));
	CBrush* oldBrush = dc.SelectObject(&brush);
	CPen* oldPen = dc.SelectObject(&pen);
	dc.RoundRect(rtFill, CPoint(radius, radius));
	...
	if (!forceNumeric && m_map.Lookup(strTemp, letters)) {
		crOldColor = dc.SetTextColor(Theme::Text);
		rtl.left += x14;
		...
		rtl.left += x12;
		rtl.right -= x4;
		crOldColor = dc.SetTextColor(Theme::TextDim);
