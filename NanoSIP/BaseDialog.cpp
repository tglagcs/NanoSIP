...
void CBaseDialog::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	if (nIDCtl == 0 && lpMeasureItemStruct->CtlType == ODT_MENU) {
		Theme::MeasureDarkMenuItem(lpMeasureItemStruct);
	}
	else {
...
void CBaseDialog::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	if (nIDCtl == 0 && lpDrawItemStruct->CtlType == ODT_MENU) {
		Theme::DrawDarkMenuItem(lpDrawItemStruct);
	}
	else {
...
BOOL CBaseDialog::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
