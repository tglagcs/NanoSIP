...
void CClosableTabCtrl::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	//
	// Tab backgrounds are always drawn flat (dark palette); the native
	// Vista/Explorer tab theme has no dark variant, so it is not used here.
	HTHEME hTheme = NULL;
	bool bVistaHotTracked = pDC->GetTextColor() == GetSysColor(COLOR_HOTLIGHT);
	bool bVistaThemeActive = IsVistaThemeActive();
	...
	COLORREF clref = bSelected ? Theme::Selection : (bVistaHotTracked ? Theme::ControlHover : Theme::Control);
	pDC->FillSolidRect(rectFill, clref);
	pDC->SetTextColor(bSelected ? Theme::Text : Theme::TextDim);
	...
	if (tci.dwState & TCIS_HIGHLIGHTED)
		crOldColor = pDC->SetTextColor(RGB(255, 90, 90));
	else if (bVistaHotTracked)
		crOldColor = pDC->SetTextColor(Theme::Text);
...
void CClosableTabCtrl::InternalInit()
{
	...
	// Visual-styles theming still paints the tab pane/background and the
	// post-WM_DRAWITEM 3D border itself (there is no dark asset for it), so
	// opt this control out of theming entirely and rely on our own flat
	// dark painting for everything.
...
BOOL CClosableTabCtrl::OnEraseBkgnd(CDC* pDC)
{
	CRect rc;
	GetClientRect(&rc);
	pDC->FillSolidRect(&rc, Theme::Window);
	return TRUE;
}
