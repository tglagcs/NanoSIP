...
BOOL CmainDlg::OnInitDialog()
{
	...
	m_mainTab.m_bCloseable = false;
	m_mainTab.SubclassDlgItem(IDC_MAIN_TAB, this);
	...
	// Themed status bars ignore SetBkColor/custom-draw colors and paint
	// their own (light) background, so opt this control out of theming.
	SetWindowTheme(m_bar.m_hWnd, L"", L"");
	statusctrl.SetBkColor(Theme::Window);
	mapRect.bottom = 12;
	...
	imageListStatus->SetBkColor(CLR_NONE);
	...
	tab->SetCurSel(min(accountSettings.activeTab, (int)tab->GetItemCount() - 1));
...
void CmainDlg::MainPopupMenu(bool isMenuButton)
{
	...
	Theme::ApplyDarkMenu(tracker->m_hMenu);
	SetForegroundWindow();
	tracker->TrackPopupMenu(0, point.x, point.y, this);
	Theme::ReleaseDarkMenuData(tracker->m_hMenu);
...
{
	if (TimerVal == IDT_TIMER_FORWARDING) {
		KillTimer(IDT_TIMER_FORWARDING);
		if (forwardingTimerCallId != PJSUA_INVALID_ID) {
...
void CmainDlg::PJCreateRaw()
{
	...
	*/
	...
	media_cfg.ec_tail_len = accountSettings.ec ? 20 : 0;

	// Only G.711 (8 kHz, mono) is used, so the media engine never needs
	// a higher clock rate or more than one channel.
...
void CmainDlg::PJAudioCodecs()
{
	...
	// Only G.711 (A-law/u-law) is offered - disable every other codec
	// the pjmedia codec manager knows about, then enable just those two.
	...
	pj_str_t pcma = pj_str("PCMA/8000/1");
	pj_str_t pcmu = pj_str("PCMU/8000/1");
...
void CmainDlg::SetPaneText2(CString str)
{
	if (str.IsEmpty()) {
		...
		if (pDC && pDC->m_hAttribDC) {
			...
			m_bar.ReleaseDC(pDC);
            width = MulDiv(size.cx + 8, dpiY, 96);
		}
		else {
			width = MulDiv(7 * str.GetLength() + 8, dpiY, 96);
		}
...
void CmainDlg::OpenTransferDlg(CWnd * pParent, msip_action action, pjsua_call_id call_id)
{
